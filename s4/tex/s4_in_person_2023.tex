...
\color{black}
% pull and clean the \texttt{~/tb\_ws/src/asl-tb3-utils} repository in the computer:
% \begin{lstlisting}
% git pull
% git reset --hard
% git clean -fdx
...
% 2023: Below commented our is already done in setup
% Copy over the following files to \texttt{tb\_ws/scripts/}

% \begin{lstlisting}
% P2_pose_stabilization.py (will change)
...
% dont think we need the commented out stuff below
% You will need to make some small modification to {\tt navigator.py} in the {\tt A\_star} class. 
% Find and replace the initialization of two variables as following, and test it locally(on your own computer, with \texttt{resolution = 0.1}) to make sure it is working as expected.
% \begin{lstlisting}
...
% KN: assume we've already created the rviz file and saved it in our git repo
Next, we'll open our saved rviz configuration from last week's section. From a new terminal, %open \texttt{rviz}. 
\begin{lstlisting}
